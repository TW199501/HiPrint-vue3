...
Object.keys(i18nModules).forEach((path) => {
  const match = path.match(/\.\.\/i18n\/([^.]+)/)
  if (match) {
    const mod = i18nModules[path]
    languages[match[1]] = mod.default || mod
  }
})
    ...
    function o(i) {
      ...
      if (!(i.ctrlKey || i.metaKey) && (i.data.target.className.startsWith('resize-panel') || "2" == i.data.target.style.zIndex || i.data.target.className.startsWith('hiprint-printElement'))) {
        ...
        if (t(".mouseRect").length == 0 && o.options.designTarget && o.options.designTarget.panel.printElements.filter(function (el) {
          return el.designTarget && "block" == el.designTarget.children().last().css("display") && !el.printElementType.type.includes("table");
        }).length <= 1) {
          ...
          (() => {
            let oPositions = o.options.designTarget.panel.printElements.filter(el => el.id != o.options.designTarget.id && el.designTarget).map(el => {
